...
export default function AcompanantesPage() {
  ...
  const compartirCodigo = async (codigo: string) => {
    const mensaje = `Este es tu código de acceso a la web de la boda: ${codigo}\n${window.location.origin}`;
    if (navigator.share) {
      try {
      ...
      try {
        ...
        alert("El código ha sido copiado al portapapeles. Puedes compartirlo manualmente.");
      } catch {
        ...
        <form className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              >
                <Share2 className="w-5 h-5 text-stone-500 hover:text-stone-800" />
              </button>
            </div>
          </div>
